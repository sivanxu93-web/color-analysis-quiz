...
export async function POST(req: NextRequest) {
  try {
    ...
    // Extract user info
    const email = body.email || body.payload?.email || object?.customer?.email || body.data?.email || body.customer_email;
    const userId = body.metadata?.user_id || body.payload?.metadata?.user_id || object?.metadata?.user_id || object?.metadata?.referenceId || body.custom_fields?.user_id;
    const plan = body.metadata?.plan || body.payload?.metadata?.plan || object?.metadata?.plan || body.custom_fields?.plan;
    const billingCycle = body.metadata?.billingCycle || body.payload?.metadata?.billingCycle || object?.metadata?.billingCycle || body.custom_fields?.billingCycle;
    ...
    const amount = body.amount || body.payload?.amount || object?.order?.amount || body.data?.amount || object?.amount;
    const amountVal = amount ? parseFloat(amount) : 0;
    const isRecurring = object?.order?.type === 'recurring' || !!object?.subscription || eventType.startsWith('subscription.');
    ...
    const subscriptionId = object?.subscription?.id || (eventType.startsWith('subscription.') ? object?.id : null);
    const subscriptionStatus = object?.subscription?.status || object?.status;
    const creemCustomerId = object?.customer?.id;
    const currentPeriodEnd = object?.subscription?.current_period_end_date || object?.current_period_end_date;
    ...
    if (creditsToAdd > 0) {
        ...
        if (creditRes.rows.length === 0) {
             ...
             const creditColumn = isSubscriptionPayment ? "subscription_credits" : "permanent_credits";
             await db.query(
                 ...
                 [
                     ...
                     subscriptionId, 
                     subscriptionStatus, 
                     isSubscriptionPayment ? (plan || (creditsToAdd >= 500 ? 'pro' : 'standard')) : null,
                     isSubscriptionPayment ? (billingCycle || 'monthly') : null,
                     creemCustomerId, 
                     currentPeriodEnd
                 ]
        ...
        await db.query(
            ...
            , [targetUserId, creditsToAdd, `Creem ${isSubscriptionPayment ? 'Subscription' : 'Purchase'}: ${eventType}`]
        );
        
        console.log(`Successfully credited ${creditsToAdd} ${isSubscriptionPayment ? 'subscription' : 'permanent'} points to user ${targetUserId}`);
        ...
        await db.query(
            `UPDATE user_available SET 
                ...
                subscription_billing_cycle = COALESCE($6, subscription_billing_cycle)
             WHERE user_id = $1`,
            [targetUserId, subscriptionStatus, currentPeriodEnd, subscriptionId, plan, billingCycle]
        );
        console.log(`Updated subscription status for user ${targetUserId} to ${subscriptionStatus}`);
    }
